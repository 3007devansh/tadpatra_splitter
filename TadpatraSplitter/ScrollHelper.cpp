...
//              https://www.codeproject.com/Articles/10902/Add-Scrolling-to-a-CWnd-or-CDialog-using-a-C-Helpe
...
void CScrollHelper::SetDisplaySize(int displayWidth, int displayHeight)
{
    ...
    if ( m_pAttachWnd != NULL && ::IsWindow(m_pAttachWnd->m_hWnd) )
        UpdateScrollInfo();
}
...
void CScrollHelper::ScrollToOrigin(bool scrollLeft, bool scrollTop)
{
    ...
    if ( scrollLeft )
    {
        if ( m_DisplaySize.cx > 0 && m_PageSize.cx > 0 && m_ScrollPos.cx > 0 )
        {
            ...
            m_pAttachWnd->SetScrollPos(SB_HORZ, m_ScrollPos.cx, TRUE);
    ...
    if ( scrollTop )
    {
        if ( m_DisplaySize.cy > 0 && m_PageSize.cy > 0 && m_ScrollPos.cy > 0 )
        {
            ...
            m_pAttachWnd->SetScrollPos(SB_VERT, m_ScrollPos.cy, TRUE);
...
BOOL CScrollHelper::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
    ...
    m_pAttachWnd->GetScrollRange(SB_VERT, &scrollMin, &scrollMax);
...
int CScrollHelper::Get32BitScrollPos(int bar, CScrollBar* pScrollBar)
{
    ...
    ASSERT( m_pAttachWnd != NULL );
    HWND hWndScroll;
    if ( pScrollBar == NULL )
        hWndScroll = m_pAttachWnd->m_hWnd;
    else
...
void CScrollHelper::UpdateScrollInfo()
{
    ...
    // expect.
    CRect rect;
    GetClientRectSB(m_pAttachWnd, rect);
...
void CScrollHelper::DoTheScroll(int cx, int cy)
{
	...
	m_pAttachWnd->ScrollWindow(cx, cy, 0, &rClip);
}
                                    ...
                                    LONG& pageSize, LONG& scrollPos, LONG& deltaPos)
{
    ...
    if ( windowSize < displaySize )
    {
        ...
        deltaPos = m_pAttachWnd->GetScrollPos(bar) - scrollPos;
    }
    else
    {
        ...
        pageSize = 0;
        scrollPos = 0;
        deltaPos = m_pAttachWnd->GetScrollPos(bar);
    }
    ...
    si.nPage  = pageSize;
    si.nPos   = scrollPos;
    m_pAttachWnd->SetScrollInfo(bar, &si, TRUE);
}
